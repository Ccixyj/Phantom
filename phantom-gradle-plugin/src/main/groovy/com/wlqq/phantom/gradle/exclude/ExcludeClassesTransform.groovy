...
import com.wlqq.phantom.gradle.dependency.AarDependenceInfo
import com.wlqq.phantom.gradle.dependency.DependenceInfo
import com.wlqq.phantom.gradle.dependency.JarDependenceInfo
...
class ExcludeClassesTransform extends Transform {
    ...
    ExcludeClassesTransform(Project project) {
        ...
        this.mWlExclude = project.extensions.getByName(Constant.USER_CONFIG)
        project.afterEvaluate {
            if (agpVersion.greaterThanOrEqualTo(Constant.AGP_3_0)) {
                mStripDependencies = computeStripDependenceListForAgp3x()
                mRelativeLibs = new HashSet<>(mStripDependencies.collect {
                    (it instanceof AarDependenceInfo) ? ((AarDependenceInfo) it).bundle.absolutePath : it.jarFile.absolutePath
                })
                excludeRes2()
            } else {
    ...
    void transform(TransformInvocation transformInvocation) throws TransformException, InterruptedException, IOException {
        ...
        File dest = transformInvocation.outputProvider.getContentLocation("ph_excluded", TransformManager.CONTENT_JARS, TransformManager.SCOPE_FULL_PROJECT, Format.JAR)
     ...
     */
    private static void writeKeepClassesToProguardFile(Set<String> classes, File proguardFile) {
    ...
    private List<DependenceInfo> computeStripDependenceListForAgp3x() {
        ...
        dependencies.libraries.each {
            ...
            if (excludeLibs.contains("${mavenCoordinates.groupId}:${mavenCoordinates.artifactId}")) {
                ...
                Log.i logtag, "Need retain aar: ${mavenCoordinates.groupId}:${mavenCoordinates.artifactId}:${mavenCoordinates.version}"
                retainedAarLibs.add(
                        new AarDependenceInfo(
                                mavenCoordinates.groupId,
                                mavenCoordinates.artifactId,
        ...
        dependencies.javaLibraries.each {
            ...
            if (excludeLibs.contains("${mavenCoordinates.groupId}:${mavenCoordinates.artifactId}")) {
                ...
                Log.i logtag, "Need retain jar: ${mavenCoordinates.groupId}:${mavenCoordinates.artifactId}:${mavenCoordinates.version}"
                retainedJarLib.add(
                        new JarDependenceInfo(
                                mavenCoordinates.groupId,
                                mavenCoordinates.artifactId,
        ...
        // write to dir <application module>/build/intermediates/phantom/<build variant>
        final def outputDir = new File(scope.globalScope.intermediatesDir, Paths.get(Constant.INTERMEDIATES_DIR, scope.variantConfiguration.dirName).toString())

        com.wlqq.phantom.gradle.utils.FileUtils.saveFile(outputDir, "stripDependencies", stripDependencies)
        com.wlqq.phantom.gradle.utils.FileUtils.saveFile(outputDir, "retainedAarLibs", retainedAarLibs)
        com.wlqq.phantom.gradle.utils.FileUtils.saveFile(outputDir, "retainedJarLib", retainedJarLib)
